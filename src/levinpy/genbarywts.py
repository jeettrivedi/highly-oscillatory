import numpy as np
from .helper_methods import range_inc
...
def Genbarywts(tau, s):
    ...
    beta = np.zeros(n)
    [u, v, w] = [np.zeros([n, s_max]), np.zeros([n, s_max + 1]), np.zeros([n, s_max])]
    ...
    for i in range_inc(0, n - 1):
        for m in range_inc(0, s_max - 1):
            u[i, m] = np.sum(strainer_array(i, n) * s * delta_tau[:, i] ** (-m - 1))
    ...
    for k in range(0, n):
        ...
        D[irow, brks[k] + 1 : brks[k] + s[k]] = -w[k, 0 : s[k] - 1] / w[k, s[k] - 1]
