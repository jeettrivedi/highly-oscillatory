...
def Levin_Bern_int(F, G, tau, omega_range):
    ...
    coef_f = Bern_Interpolate(jt.Cheb_nodes(a, b, 2*n+1),
                              F(jt.Cheb_nodes(a, b, 2*n+1)),
                              a,
                              b)
    coef_one = np.ones(n+1)
    coef_g = Bern_Interpolate(tau,
                              G(tau),
                              a,
                              b)
    ...
    for w in omega_range:
        A = (np.dot(Mul_Operator_Matrix(coef_one), D)
             + np.complex(0, w)*Mul_Operator_Matrix(coef_dg))
