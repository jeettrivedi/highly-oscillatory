...
from sympy import diff,limit,evalf,sqrt,sin,cos,root,log,exp,simplify,lambdify,Symbol
...
def Coeff_to_Poly(coeff_list,x,a=0,b=1):
	'''
	"Synthesis" Operator: Given a sequence of coefficients, this returns the value of the 
...
def Levin_Bern_int(F,G,tau,omega_range):
	...
	coef_f = Bern_Interpolate(jt.Cheb_nodes(a,b,2*n+1),F(jt.Cheb_nodes(a,b,2*n+1)),a,b)
	...
	for w in omega_range: 
		A = np.dot(Mul_Operator_Matrix(coef_one),D)+np.complex(0,w)*Mul_Operator_Matrix(coef_dg)
		...
		Levin[k] = sol[-1]*np.exp(complex(0,w)*G(tau[-1]))-sol[0]*np.exp(complex(0,w)*G(tau[0]))
		k+=1
	return Levin
