...
# Non regularized version
	...
	for i in range(1,N-1):
		...
		B[i,i+1] = -(4*h[i]+2*h[i-1])*h[i-1]**2/(h[i]+h[i-1])**3/h[i]**3

	A[0,0] = 1/(h[0]+h[1])/(h[0]+h[1]+h[2])
	A[0,1] = 1/h[1]/(h[1]+h[2])
	B[0,0] = ((4*h[0]**2+6*h[0]*h[1]+3*h[0]*h[2]+2*h[1]**2+2*h[1]*h[2])
	...
	B[-1,-4] = -h[-1]**2/(h[-3]+h[-2]+h[-1])**2/(h[-3]+h[-2])**2/h[-3]
	B[-1,-3] = h[-1]**2/(h[-2]+h[-1])**2/h[-2]**2/h[-3]
	B[-1,-2] = (1/h[-1]*((2*h[-2]-h[-1])*h[-3]+2*h[-2]*(h[-2]-h[-1]))/h[-2]**2/
				(h[-3]+h[-2])**2
				)
	B[-1,-1] = -((4*h[-1]**2+(6*h[-2]+3*h[-3])*h[-1]+2*h[-2]*(h[-3]+h[-2]))
...
# Package imports
	...
	import matplotlib.pyplot as pl
	from sympy import diff,Symbol,limit,evalf,sqrt,sin,cos,root,log,exp,simplify,N
	...
	for omega in omega_range:
		...
		P = jt.TSVD_Solve(B+np.complex(0,omega)*np.dot(A,g_matrix),np.dot(A,p),1e-13)
		Levin[i] = (P[-1]*np.exp(np.complex(0,omega)*float(N(g(tau[-1]))))-
					P[0]*np.exp(np.complex(0,omega)*float(N(g(tau[0])))))
		print(Levin[i])
		i+=1
	return Levin